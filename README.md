...
发布包中的 `add-to-system-path.bat` 总会将发布目录添加到当前用户 `PATH`；如果它已经以管理员权限运行，还会同时添加到系统 `PATH`。脚本不会主动请求提权，更新后需要重新打开终端。执行 `add-to-system-path.bat --dry-run` 可预览操作而不修改任何 PATH。
